...
class L2Adversary(object):
    ...
    def __init__(self, targeted=True, confidence=0.0, c_range=(1e-3, 1e10),
                 ...
                 box=(-1., 1.), optimizer_lr=1e-2, init_rand=False, num_classes=10, device=torch.device('cpu')):
        """
    ...
    def __call__(self, model, inputs, targets, to_numpy=True):
        ...
        targets_oh = torch.zeros(targets.size() + (self.num_classes,)).to(self.device)
        ...
        for sstep in range(self.binary_search_steps):
            ...
            scale_consts = torch.from_numpy(
                np.copy(scale_consts_np)).float().to(self.device)  # type: torch.FloatTensor
            scale_consts = runutils.make_cuda_consistent(
                model, scale_consts)[0]
            ...
            for optim_step in range(self.max_steps):
                batch_loss, pert_norms_np, pert_outputs_np, advxs_np = \
                    self._optimize(model, optimizer, inputs_tanh_var,
                                   ...
                                   scale_consts_var)
                # if (optim_step % 500 == 0) and (optim_step != 0):
                #     # FIXME
            ...
            for i in range(batch_size):
                ...
                if best_l2_ppred[i] != -1:
                    ...
                    if upper_bounds_np[i] < self.c_range[1] * 0.1:
                        scale_consts_np[i] = (
                            lower_bounds_np[i] + upper_bounds_np[i]) / 2
                else:
    ...
    def _optimize(self, model, optimizer, inputs_tanh_var, pert_tanh_var,
        ...
        advxs_var = self._from_tanh_space(
            inputs_tanh_var + pert_tanh_var)  # type: Variable
        ...
        batch_loss_var = torch.sum(
            perts_norm_var + c_var * f_var)  # type: Variable
